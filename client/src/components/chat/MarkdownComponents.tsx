...
export const markdownComponents: Components = {
    h1: ({ children }) => <h1 className="text-2xl font-bold text-white mb-4 break-all">{children}</h1>,
    h2: ({ children }) => <h2 className="text-xl font-bold text-white mb-3 break-all">{children}</h2>,
    h3: ({ children }) => <h3 className="text-lg font-bold text-white mb-2 break-all">{children}</h3>,
    p: ({ children, ...props }) => (
        <p {...props} className="text-sm text-gray-100 mb-4 last:mb-0 break-all">
            {children}
        </p>
    ),
    strong: ({ children }) => <strong className="font-bold text-white break-all">{children}</strong>,
    em: ({ children }) => <em className="italic text-gray-300 break-all">{children}</em>,
    a: ({ children, ...props }) => (
        <a
            {...props}
            target="_blank"
    ...
    ol: ({ children, ...props }) => (
        <ol {...props} className="list-decimal list-inside space-y-1 mb-4 last:mb-0 break-all">
            {children}
        </ol>
    ),
    li: ({ children, ...props }) => (
